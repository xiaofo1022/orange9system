...
CREATE TRIGGER `WEB_PIC_INSERT` BEFORE INSERT ON `web_pic` FOR EACH ROW SET NEW.INSERT_DATETIME = NOW(), NEW.UPDATE_DATETIME = NOW();
CREATE TRIGGER `WEB_PIC_UPDATE` BEFORE UPDATE ON `web_pic` FOR EACH ROW SET NEW.UPDATE_DATETIME = NOW();