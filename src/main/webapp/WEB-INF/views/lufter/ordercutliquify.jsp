...
<div class="row">
	...
	<jsp:include page="panel.jsp" flush="true">
		<jsp:param name="link" value="cutliquify"/>
	</jsp:include>
...
<script>
	...
	function skipAllot(orderId) {
		...
		if (result) {
			$.post("<c:url value='/orderPostProduction/skipAllotCutLiquify/" + orderId + "'/>", null, function(data, status) {
	...
	function nextStep(orderId, operatorId) {
		var result = confirm("是否确认完成修片？");
		if (result) {
			$.post("<c:url value='/orderPostProduction/setCutLiquifyDone/" + orderId + "/" + operatorId + "'/>", null, function(data, status) {
				if (data.status == "success") {
					location.reload(true);
				}
			});
