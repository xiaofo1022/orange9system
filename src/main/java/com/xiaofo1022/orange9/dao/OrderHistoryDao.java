...
public class OrderHistoryDao {
  ...
  public void addOrderHistory(Order order, String info, String reason) {
    Date now = new Date();
    commonDao.insert("INSERT INTO ORDER_HISTORY (INSERT_DATETIME, UPDATE_DATETIME, ORDER_ID, OPERATOR_ID, INFO, REMARK) VALUES (?, ?, ?, ?, ?, ?)", now, now,
        order.getId(), order.getUserId(), info, reason);
  }
