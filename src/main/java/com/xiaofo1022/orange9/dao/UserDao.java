...
public class UserDao {
  ...
  public void insertUser(User user) {
    ...
    int id = commonDao.insert(
        ...
        user.getName(), user.getRoleId(), user.getAccount(), user.getPassword(), user.getSalary(), user.getPerformancePay(), user.getHeader(), user.getBossId(),
        now, now, user.getPhone(), user.getIsAdmin());
    user.setId(id);
  }
  ...
  public List<User> getUserList(int bossId) {
    return commonDao.query(User.class,
        "SELECT A.* FROM USER A LEFT JOIN ROLE B ON A.ROLE_ID = B.ID WHERE A.ACTIVE = 1 AND B.IS_SHOW = 1 AND A.BOSS_ID = ? ORDER BY A.ROLE_ID, A.ID", bossId);
  }
  ...
  public void updateUser(User user) {
    commonDao.update(
        "UPDATE USER SET UPDATE_DATETIME = ?, NAME = ?, ROLE_ID = ?, PHONE = ?, SALARY = ?, PERFORMANCE_PAY = ?, IS_ADMIN = ?, HEADER = ? WHERE ID = ?",
        new Date(), user.getName(), user.getRoleId(), user.getPhone(), user.getSalary(), user.getPerformancePay(), user.getIsAdmin(), user.getHeader(),
        user.getId());
  }
