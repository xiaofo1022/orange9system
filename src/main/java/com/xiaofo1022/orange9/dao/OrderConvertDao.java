...
public class OrderConvertDao {
  ...
  public void insertOrderConvert(int orderId, int userId) {
    Date now = new Date();
    commonDao.insert("INSERT INTO ORDER_CONVERT_IMAGE (ORDER_ID, INSERT_DATETIME, UPDATE_DATETIME, OPERATOR_ID) VALUES (?, ?, ?, ?)", orderId, now, now,
        userId);
  }
  ...
  public List<OrderConvertImage> getOrderConvert(int ownerId) {
    return commonDao.query(OrderConvertImage.class,
        "SELECT A.* FROM ORDER_CONVERT_IMAGE A LEFT JOIN ORDERS B ON A.ORDER_ID = B.ID WHERE A.IS_DONE = 0 AND B.OWNER_ID = ?", ownerId);
  }
