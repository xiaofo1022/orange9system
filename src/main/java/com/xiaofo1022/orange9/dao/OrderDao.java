...
public class OrderDao {
	...
	public void insertOrder(Order order) {
		Date now = new Date();
		int id = commonDao.insert("INSERT INTO ORDERS (INSERT_DATETIME, UPDATE_DATETIME, SHOOT_DATE, CLIENT_ID, MODEL_NAME, BROKER_NAME, BROKER_PHONE, SHOOT_HALF, DRESSER_NAME, STYLIST_NAME, PHOTOGRAPHER_ID, ASSISTANT_ID, STATUS_ID, OWNER_ID) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)",
				now, now, order.getShootDate(), order.getClientId(), order.getModelName(), order.getBrokerName(), order.getBrokerPhone(), order.getShootHalf(), order.getDresserName(), order.getStylistName(), order.getPhotographerId(), order.getAssistantId(), OrderStatusConst.SHOOTING, order.getOwnerId());
		order.setId(id);
	...
	public List<Order> getOrderListByClient(int clientId) {
		return commonDao.query(Order.class, "SELECT * FROM ORDERS WHERE CLIENT_ID = ? AND STATUS_ID >= ? AND ACTIVE = 1 ORDER BY STATUS_ID ASC, ID DESC", clientId, OrderStatusConst.WAITING_FOR_CLIENT_CHOSE);
	}
	
	public List<Order> getOrderListByDate(String startDate, String endDate, int ownerId) {
		return commonDao.query(Order.class, "SELECT * FROM ORDERS WHERE ACTIVE = 1 AND SHOOT_DATE BETWEEN '" + startDate + " 00:00:00' AND '" + endDate + " 00:00:00' AND OWNER_ID = ? ORDER BY SHOOT_DATE, SHOOT_HALF", ownerId);
	}
	...
	public List<Order> getOrderListByStatus(int statusId, int ownerId) {
		return commonDao.query(Order.class, "SELECT * FROM ORDERS WHERE ACTIVE = 1 AND STATUS_ID = ? AND OWNER_ID = ? ORDER BY SHOOT_DATE, SHOOT_HALF", statusId, ownerId);
	}
