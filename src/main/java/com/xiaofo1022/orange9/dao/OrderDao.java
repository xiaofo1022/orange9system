...
import com.xiaofo1022.orange9.common.OrderStatusMsg;
import com.xiaofo1022.orange9.common.StatusMsg;
...
public class OrderDao {
	...
	public void insertOrder(Order order) {
		...
		int id = commonDao.insert("INSERT INTO ORDERS (INSERT_DATETIME, UPDATE_DATETIME, SHOOT_DATE, CLIENT_ID, MODEL_NAME, BROKER_NAME, BROKER_PHONE, SHOOT_HALF, DRESSER_NAME, STYLIST_NAME, PHOTOGRAPHER_ID, ASSISTANT_ID, STATUS_ID) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)",
				now, now, order.getShootDate(), order.getClientId(), order.getModelName(), order.getBrokerName(), order.getBrokerPhone(), order.getShootHalf(), order.getDresserName(), order.getStylistName(), order.getPhotographerId(), order.getAssistantId(), StatusMsg.SHOOTING);
		order.setId(id);
		orderHistoryDao.addOrderHistory(order, OrderStatusMsg.CREATE_ORDER);
	}
	...
	public void updateOrderStatus(Order order, OrderStatus orderStatus) {
		...
		orderHistoryDao.addOrderHistory(order, OrderStatusMsg.STATUS_CHANGE + orderStatus.getName());
	}
}
