...
public class OrderTransferDao {
  ...
  public void insertOrderTransfer(int orderId, int userId) {
    Date now = new Date();
    commonDao.insert("INSERT INTO ORDER_TRANSFER_IMAGE (ORDER_ID, INSERT_DATETIME, UPDATE_DATETIME, OPERATOR_ID) VALUES (?, ?, ?, ?)", orderId, now, now,
        userId);
  }
  ...
  public List<OrderTransferImage> getOrderTransferImageList(int ownerId) {
    List<OrderTransferImage> transferList = commonDao.query(OrderTransferImage.class,
        "SELECT A.* FROM ORDER_TRANSFER_IMAGE A LEFT JOIN ORDERS B ON A.ORDER_ID = B.ID WHERE A.IS_DONE = 0 AND B.OWNER_ID = ? ORDER BY A.INSERT_DATETIME DESC",
        ownerId);
  ...
  public void insertOrderTransferImageData(OrderTransferImageData transferImageData) {
    Date now = new Date();
    int id = commonDao.insert("INSERT INTO ORDER_TRANSFER_IMAGE_DATA (INSERT_DATETIME, UPDATE_DATETIME, ORDER_ID, FILE_NAME) VALUES (?, ?, ?, ?)", now, now,
  ...
  public boolean isExistTransferImageData(int orderId, String fileName) {
    Count count = commonDao.getFirst(Count.class, "SELECT COUNT(ID) AS CNT FROM ORDER_TRANSFER_IMAGE_DATA WHERE ORDER_ID = ? AND FILE_NAME = ?", orderId,
        fileName);
  ...
  public void setTransferImageSelectedByFile(int orderId, String fileName) {
    commonDao.update("UPDATE ORDER_TRANSFER_IMAGE_DATA SET IS_SELECTED = 1, UPDATE_DATETIME = ? WHERE ORDER_ID = ? AND FILE_NAME = ?", new Date(), orderId,
        fileName);
  }
  ...
  public void setImageDataFixSkin(int imageId, int fixSkinId) {
    commonDao.update("UPDATE ORDER_TRANSFER_IMAGE_DATA SET FIX_SKIN_ID = ?, UPDATE_DATETIME = ? WHERE ID = ?", fixSkinId, new Date(), imageId);
  }
}
