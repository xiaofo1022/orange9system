package com.xiaofo1022.orange9.common;

public class RoleConst {
	public static final int BOSS_ID = 1;
	public static final int PHOTOGRAPHER_ID = 2;
	public static final int DESIGNER_ID = 3;
	public static final int ASSISTANT_ID = 4;
	public static final int CLIENT_ID = 5;
	public static final String DEFAULT_PASSWORD = "666666";
	public static final String COLUMN_PHOTOGRAPHER = "PHOTOGRAPHER_ID";
	public static final String COLUMN_ASSISTANT = "ASSISTANT_ID";
}
